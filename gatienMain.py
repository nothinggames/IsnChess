from tkinter import *
from gatienClasses import *
from gatienFonctions import *
from time import *
...
class Bouton():  # Cette classe sert à créer et stocker chaque bouton avec ses coordonnées (O, i, j). O est situé en bas à gauche de la fenetre
	...
	def __init__(self, i, j, frame_plateau, text=""):
		...
		self.bouton = Button(frame_plateau, background=self.couleur_cases(),
							 command=lambda: appel_bouton(plateau, self.i, self.j),
							 width=10, height=5, image="", bd=0,
							 activebackground=self.couleur_cases(),
							 relief=SUNKEN)  # Chaque case est un bouton qui appel la fonction appel_bouton(i, j)
		# self.bouton.bind("<Enter>", self.bouton_entree)
		# self.bouton.bind("<Leave>", self.bouton_sortie)
		self.bouton_grid()

	def changer_image(self,
					  img):  # Sert à modifier l'image à afficher lors du placement des pièces ou lors d'un déplacement.
...
def appel_bouton(plateau, i,
				 j):  # Fonction appelé lors d'un clic sur un bouton, les coordonnées (i, j) donnent le bouton activé
	# print(plateau[f'{i}{j}
